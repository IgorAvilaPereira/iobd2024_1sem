...
INSERT INTO usuario (nome, email, senha, data_nascimento) values
('teste', 'teste@riogrande.ifrs.edu.br', md5('123'), '1987-01-20');

INSERT INTO conta (nome_usuario, usuario_id) VALUES
('teste_usuario', 3);


CREATE VIEW ajuda_gunther AS
select usuario.id, usuario.nome, count(*) as qtde from usuario inner join conta on (usuario.id = conta.usuario_id) group by  usuario.id, usuario.nome having count(*) = (select count(*) from usuario inner join conta on (usuario.id = conta.usuario_id) group by usuario.id order by count(*) desc limit 1) order by usuario.id ;


INSERT INTO usuario (nome, email, senha, data_nascimento) values
('teste_sem_conta', 'teste_sem_conta@riogrande.ifrs.edu.br', md5('123'), '1987-01-20');
